...
# atur output & jumlah data
OUTPUT_CSV = "nyarigawe_reviews.csv"
TARGET_COUNT = 1000          # target ambil (sebelum buang netral)
LANG = "id"
COUNTRY = "id"
SLEEP_MS = 250               # jeda per request (biar aman)
...
def extract_app_id(playstore_url: str) -> str:
    ...
    if not app_id:
        raise ValueError("URL tidak valid. Pastikan ada '?id=' di link Play Store.")
    return app_id
...
def scrape_reviews(app_id: str, count: int):
    ...
    while fetched < count:
        batch_size = min(200, count - fetched)  # max 200 per request
...
if __name__ == "__main__":
    ...
    print("[INFO] app_id terdeteksi:", app_id)

    df = scrape_reviews(app_id, TARGET_COUNT)
    df.to_csv(OUTPUT_CSV, index=False, encoding="utf-8-sig")

    print("[DONE] File tersimpan:", OUTPUT_CSV)
