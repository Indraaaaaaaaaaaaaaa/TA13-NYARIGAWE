...
# =========================
# INFO STUDI KASUS (GANTI SESUAI APP)
# =========================
APP_NAME = "Nyarigawe"
APP_ID = "id.diskominfo.nyarigawe"
PLAYSTORE_URL = "https://play.google.com/store/apps/details?id=id.diskominfo.nyarigawe"
...
def load_model(path: str, file_mtime: float):
    ...
    file_mtime: waktu modifikasi file untuk invalidate cache saat model berubah.
    """
    try:
        model = joblib.load(path)
        # Verifikasi model sudah fitted
        if hasattr(model, 'named_steps'):
            if 'tfidfvectorizer' in model.named_steps:
                tfidf = model.named_steps['tfidfvectorizer']
                # Test transform untuk memastikan fitted
                try:
                    _ = tfidf.transform(["test"])
                except Exception:
                    st.error("Model TF-IDF belum fitted! Silakan retrain model.")
                    raise
        return model
...
# =========================
try:
    # Dapatkan waktu modifikasi file untuk invalidate cache saat model berubah
    if os.path.exists(MODEL_PATH):
        file_mtime = os.path.getmtime(MODEL_PATH)
    else:
        file_mtime = 0
    
    model = load_model(MODEL_PATH, file_mtime)
    st.success(f"Model berhasil dimuat: {MODEL_PATH}")
except Exception as e:
    st.error(f"Gagal load model: {MODEL_PATH}\n\nDetail: {e}")
    st.info("💡 **Tips:** Jika model baru saja di-retrain, coba:\n"
            "1. Stop aplikasi Streamlit (Ctrl+C)\n"
            "2. Restart aplikasi Streamlit\n"
            "3. Atau klik tombol 'R' di browser untuk refresh halaman")
    st.stop()
